...
describe('holdTapLayer', () => {
  ...
  it('should set optional modifiers', () => {
    const rule = holdTapLayer('a').optionalModifiers(['shift', 'control']).build();
    const manipulator = rule.manipulators[0] as any;
    expect(manipulator.from.modifiers).toEqual({ optional: ['shift', 'control'] });
  });
  ...
  it('should add an onHold event', () => {
    ...
    expect(manipulator.to_if_held_down).toEqual(expect.arrayContaining([
  ...
  it('should add a holdOnOtherKeyPressManipulator', () => {
    const rule = holdTapLayer('a').holdOnOtherKeyPressManipulator(map('b').to('c')).build();
    ...
    expect(manipulator.conditions).toEqual([{ type: 'variable_if', name: 'layer-a', value: 1 }]);
  });
  ...
  it('should add an echoKey', () => {
    ...
    expect(manipulator.conditions).toEqual(expect.arrayContaining([
  ...
  it('should add a slowManipulator', () => {
    ...
    expect(unlessManipulator.conditions).toEqual(expect.arrayContaining([
      { type: 'variable_if', name: 'layer-a', value: 1 },
    ...
    expect(ifManipulator.conditions).toEqual(expect.arrayContaining([
      { type: 'variable_if', name: 'layer-a', value: 1 },
      { type: 'variable_if', name: 'layer-a-start', value: 1 },
    ]));
  ...
  it('should add a permissiveHoldManipulator', () => {
    const rule = holdTapLayer('a').permissiveHoldManipulator(map('b').to('c')).build();
    expect(rule.manipulators).toHaveLength(3); // config, unless, if
    ...
    expect(ifManipulator.to_after_key_up).toEqual([
      { key_code: 'c', conditions: [{ type: 'variable_if', name: 'layer-a', value: 1 }] },
      { conditions: [{ type: 'variable_unless', name: 'layer-a', value: 1 }], key_code: 'a' },
      { conditions: [{ type: 'variable_unless', name: 'layer-a', value: 1 }], key_code: 'b' },
      { set_variable: { name: 'layer-a-start', value: 0 } },
    ]);
  });
});
